...
class dataset(Dataset):
    def __init__(self, hdr_path=None, ldr_path=None, crf_list=train_crf_list, aug=True, aug_ev=4, image_size=512, stage=1, sigma=2.0):
    ...
    def __getitem__(self, index):
        ...
        # -----Hallucination-----
        hdr_clip, over_exposure_mask, under_exposure_mask = self.dynamic_clipping(hdr_image)
        ...
        if self.stage == 4: # Refinement Stage
            return {"target": transforms.functional.crop(torch.from_numpy(np.transpose(hdr_image, (2, 0, 1))), i, j, h, w),
                    ...
                    # "source_edge": transforms.functional.crop(torch.from_numpy(np.transpose(jpeg_edge, (2, 0, 1))), i, j, h, w),
                    # "target_edge": transforms.functional.crop(torch.from_numpy(np.transpose(hdr_edge, (2, 0, 1))), i, j, h, w),
                    }
