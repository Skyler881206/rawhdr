...
class PatchEmbedding(nn.Module):
    def __init__(self, in_channels, patch_size, embedding_dim):
        ...
        self.linear_projection = nn.Linear(patch_size * patch_size * in_channels, embedding_dim)

    def forward(self, x):
        B, C, H, W = x.shape
        P = self.patch_size
        assert H % P == 0 and W % P == 0, "Image dimensions must be divisible by the patch size."
        patches = x.unfold(2, P, P).unfold(3, P, P)  # B, C, num_patches_h, num_patches_w, P, P
        patches = patches.contiguous().view(B, C, -1, P * P)  # B, C, num_patches, P*P
        patches = patches.permute(0, 2, 1, 3)  # B, num_patches, C, P*P
        patches = patches.contiguous().view(B, -1, C * P * P)  # B, num_patches, C*P*P
        embeddings = self.linear_projection(patches)  # B, num_patches, embedding_dim
        return embeddings
...
class global_spatial_guidance(nn.Module):
    def __init__(self, in_channels = 4, patch_size = 16, dim = 768):
        ...
        self.down1 = Downsampling(dim, dim)
        ...
        self.LeWin_middle = nn.Sequential(
            LeWinTransformerBlock(dim=dim, input_resolution=(128, 128), num_heads=12),
            LeWinTransformerBlock(dim=dim, input_resolution=(128, 128), num_heads=12)
        )
        
        self.up1 = Upsampling(dim, dim)
        self.LeWin_up_1 = nn.Sequential(
            LeWinTransformerBlock(dim=dim, input_resolution=(256, 256), num_heads=12),
            LeWinTransformerBlock(dim=dim, input_resolution=(256, 256), num_heads=12)
        )
        
        self.up2 = Upsampling(dim, dim)
        self.LeWin_up_2 = nn.Sequential(
            LeWinTransformerBlock(dim=dim, input_resolution=(512, 512), num_heads=12),
            LeWinTransformerBlock(dim=dim, input_resolution=(512, 512), num_heads=12)
        )
        
        self.output_projection = nn.Linear(dim, in_channels * patch_size * patch_size)
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), x.size(2), int(x.size(1) ** 0.5), int(x.size(1) ** 0.5))
        x = self.down1(x)
        x = x.view(x.size(0), -1, x.size(1)) # Downsampling and Reshape to the embedding shape
        
        x = self.LeWin_down_2(x)
        x = x.view(x.size(0), x.size(2), int(x.size(1) ** 0.5), int(x.size(1) ** 0.5))
        x = self.down2(x)
        x = x.view(x.size(0), -1, x.size(1)) # Downsampling and Reshape to the embedding shape
        
        x = self.LeWin_middle(x)
